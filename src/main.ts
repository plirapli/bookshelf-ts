...
document.addEventListener(RENDER_EVENT, () => {
  const incompleteBookLength = books.filter(
    (book: Book) => !book.isComplete
  ).length;
  const completeBookLength = books.filter(
    (book: Book) => book.isComplete
  ).length;
...
const addBook = () => {
  const book: Book = {
    ...
    author: (<HTMLInputElement>document.querySelector('#author')).value,
    year: parseInt((<HTMLInputElement>document.querySelector('#year')).value),
    isComplete: (<HTMLInputElement>document.querySelector('#isComplete'))
      .checked,
    id: generateId(),
  };
  books.push(book);
...
const makeBook = ({ id, title, author, year, isComplete }: Book) => {
  let output = `
...
// Buat Nyari Buku
const findBookId = (id: number) => books.find((book) => book.id === id) || null;
const findBookTitle = (keyword: string) =>
  books.filter((book: Book) =>
    keyword ? book.title.toLowerCase().includes(keyword) : book
  );
const findBookIndex = (id: number) => {
  for (const index in books) {
    if (books[index].id === id) return parseInt(index);
  }
  return -1;
};

// Buat tombol selesai
const btnCheckHandler = (id: number) => {
...
// Buat tombol hapus
const btnDeleteHandler = (id: number) => {
...
const searchBook = () => {
  const keyword = (<HTMLInputElement>(
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Buat form input
  const submitForm = document.getElementById('inputBook') as HTMLFormElement;
  submitForm.addEventListener('submit', (e) => {
    ...
    (<HTMLInputElement>document.querySelector('#isComplete')).checked = false;
  });
